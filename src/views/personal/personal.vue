<template>
  <div>
    <div>
      <h1></h1>
         <div class="bigBox">
          <div class="big">
            <van-row>
              <van-col class="tupian">
                 <img v-if="!userInfo.headIcon" src="data:image/png;base64,">
                 <img :src="userInfo.headIcon" alt="" v-if="userInfo.headIcon">
              </van-col>
              <van-col class="denglu">
                <router-link to="/login">
                  <span v-show="!userInfo.headIcon">  
                    登录&nbsp;/&nbsp;注册
                  </span>
                  <span v-show="userInfo.headIcon">  
                    {{userInfo.userId}}
                  </span>
                </router-link>
              </van-col>
              
            </van-row>
          </div>
          <div>
            <van-grid clickable :column-num="3" >
              <van-grid-item icon="search" url="/vant/mobile.html">
              <van-icon class-prefix="iconfont icon-wuliu" size="1.5rem"/>
              <span style="font-size: 12px">我的订单</span></van-grid-item>
              <van-grid-item icon="search" url="/vant/mobile.html">
              <van-icon class-prefix="iconfont icon-kechengguanli" size="1.5rem"/>
              <span style="font-size: 12px">我的课程</span></van-grid-item>
              <van-grid-item icon="search" url="/vant/mobile.html">
              <van-icon class-prefix="iconfont icon-kechengguanli" size="1.5rem"/>
              <span style="font-size: 12px">兑换课程</span></van-grid-item>
            </van-grid>
            </div>
            <div>
              <h3>账户</h3>
            <van-grid clickable :column-num="4" >
              <van-grid-item icon="search" url="/vant/mobile.html">
              <van-icon class-prefix="iconfont icon--wodeyouhuijuan" size="1.5rem" color="rgba(255,0,0,.6)"/>
              <span style="font-size: 12px; margin-top: 15px">优惠券</span></van-grid-item>
              <van-grid-item icon="search" url="/vant/mobile.html">
              <van-icon class-prefix="iconfont icon-dizhiguanli" size="1.5rem" color="rgba(255,0,0,.6)"/>
              <span style="font-size: 12px; margin-top: 15px">地址管理</span></van-grid-item>
            </van-grid>
            </div>
            <div>
              <h3>学习</h3>
            <van-grid clickable :column-num="4" >
              <van-grid-item icon="home-o" to="/">
              <van-icon class-prefix="iconfont icon-shoucangcopy" size="1.5rem" color="rgba(0,100,0,.6)"/>
              <span style="font-size: 12px; margin-top: 15px">收藏</span></van-grid-item>
              <van-grid-item icon="search" url="/vant/mobile.html">
              <van-icon class-prefix="iconfont icon-yijianfankui" size="1.5rem" color="rgba(0,100,0,.6)"/>
              <span style="font-size: 12px; margin-top: 15px">学习记录</span></van-grid-item>
              <van-grid-item icon="search" url="/vant/mobile.html">
              <van-icon class-prefix="iconfont icon-zhushou" size="1.5rem" color="rgba(0,100,0,.6)"/>
              <span style="font-size: 12px; margin-top: 15px">成长助手</span></van-grid-item>
            </van-grid>
            </div>
            <div>
              <h3>帮助</h3>
            <van-grid clickable :column-num="4" >
              <van-grid-item icon="home-o" to="/">
              <van-icon class-prefix="iconfont icon-kefu" size="1.5rem" color="rgba(72,61,139,.6)"/>
              <span style="font-size: 12px; margin-top: 15px">在线客服</span></van-grid-item>
              <van-grid-item icon="search" url="/vant/mobile.html">
              <van-icon class-prefix="iconfont icon-yijianfankui" size="1.5rem" color="rgba(72,61,139,.6)"/>
              <span style="font-size: 12px; margin-top: 15px">意见反馈</span></van-grid-item>
              <van-grid-item icon="home-o" url="/vant/mobile.html">
              <van-icon class-prefix="iconfont icon-shezhi" size="1.5rem" color="rgba(72,61,139,.6)"/>
              <span style="font-size: 12px; margin-top: 15px">设置</span></van-grid-item>
            </van-grid>
            </div>
        </div>
    </div>
  </div>
</template>
<script>
import Vue from 'vue';
import "@/assets/font/iconfont.css";
import { Col, Row,Grid, GridItem,Icon} from 'vant';
Vue.use(Col);
Vue.use(Row);
Vue.use(Grid);
Vue.use(GridItem);
Vue.use(Icon);
export default {
  data(){
    return {
      userInfo:'',
    }
  },
  created(){
     if(this.$store.state.user._token){
      this.$http.get('http://127.0.0.1:2004/get_user_info').then(ret=>{
        console.log(ret)
        this.userInfo = ret.data
      })
    }
  }
 
}
</script>
<style scoped>
h1{
  width: 100%;
  height:100px;
  background-color: rgba(255,69,0,.6);
  margin-top: 0;
  margin-bottom: 0;
}
.bigBox{
  margin-top:-50px;
  padding:20px 10px 50px 10px;
}
.big{
  padding:15px;
  box-sizing: border-box;
  /* background-color:whitesmoke; */
  background-color:white;
  border-radius: 5px;
}
.big>van-row{
  width:100%;
  padding: 0;
}
.tupian{
  width: 54px;
  height: 54px;
  margin-left:5px;
  margin-right: 15px;
}
.tupian>img{
  display:block;
  width: 54px;
  height: 54px;
}
.denglu{
  width: 200px;
  height: 50px;
  line-height: 50px;
  margin-left: 5px;
  letter-spacing: 2px;
  font-size: 18px;
}
.denglu>a{
  color: black;
}
h3{
  font-size: 16px;
  font-weight: 600;
}

</style>